...
public class DayNightLightMode extends LightMode {
    ...
    public DayNightLightMode(GameLevel gameLevel, World world) {
    ...
    public void render(GameContext gameContext) {
        ...
        color.mul(MathUtils.lerp(.4f, .85f, currentStage()));
        rayHandler.setAmbientLight(color);
        // світло факелів
        torchLights.forEach(e -> e.setColor(
            new Color(1f, .3f, 0f, 1 - MathUtils.lerp(.3f, .65f, currentStage()))
        ));
